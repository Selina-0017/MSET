...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 256 : index
    %c0xFF = arith.constant 255 : i8
    %c0_i32 = arith.constant 0 : i32
    // locals

    %parent = memref.alloca() : memref<264xi8>

    %parent_origin = memref.subview %parent[0][8][1] : memref<264xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_target = memref.subview %parent[256][8][1] : memref<264xi8> to memref<8xi8, strided<[1], offset: 256>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_target[%i] : memref<8xi8, strided<[1], offset: 256>>
    }
    %distance_negated = arith.subi %c0, %distance : index
    %use_val_parent_target = memref.load %parent_target[%c0] : memref<8xi8, strided<[1], offset: 256>>
    ...
    ^bb0(%i: index):
      memref.store %c0xFF, %parent_target[%i] : memref<8xi8, strided<[1], offset: 256>>
