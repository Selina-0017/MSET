...
module {
  ...
  func.func @f() -> i32 {
    ...
    %parent_origin = memref.subview %parent[0][8][1] : memref<1012xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_target = memref.subview %parent[1004][8][1] : memref<1012xi8> to memref<8xi8, strided<[1], offset: 1004>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_target[%i] : memref<8xi8, strided<[1], offset: 1004>>
    }
    %distance_negated = arith.subi %c0, %distance : index
    %use_val_parent_target = memref.load %parent_target[%c0] : memref<8xi8, strided<[1], offset: 1004>>
    ...
    scf.for %reach_index = %c0 to %distance step %c1 {
      ...
      %val = memref.load %parent_origin[%index] : memref<8xi8, strided<[1], offset: 0>>
