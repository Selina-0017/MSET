...
module {
  ...
  func.func @f() -> memref<8xi8> {
    ...
    scf.for %i = %c0 to %distance step %c1024 {
      ...
      %dst_slice = memref.subview %read_value_294[%c0][%step][1] : memref<1024xi8> to memref<?xi8, strided<[1], offset: ?>>
      memref.copy %src_slice, %dst_slice : memref<?xi8, strided<[1], offset: ?>> to memref<?xi8, strided<[1], offset: ?>>
    }
    %use_val_read_value_294 = memref.load %read_value_294[%c0] : memref<1024xi8>
    func.call @use(%use_val_read_value_294) : (i8) -> ()
    %read_value_296 = memref.alloca() : memref<8xi8>
    memref.copy %s_origin, %read_value_296 : memref<8xi8, strided<[1], offset: 8>> to memref<8xi8>
    %use_val_read_value_296 = memref.load %read_value_296[%c0] : memref<8xi8>
    func.call @use(%use_val_read_value_296) : (i8) -> ()
