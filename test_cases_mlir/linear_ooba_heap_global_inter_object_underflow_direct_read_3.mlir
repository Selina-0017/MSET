...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 771 : index
    %c0_i32 = arith.constant 0 : i32
    // locals


    %parent = memref.alloc() : memref<779xi8>
    %parent_target = memref.subview %parent[0][8][1] : memref<779xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_origin = memref.subview %parent[771][8][1] : memref<779xi8> to memref<8xi8, strided<[1], offset: 771>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_target[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 771>>
    }
    ...
    %use_val_parent_origin = memref.load %parent_origin[%c0] : memref<8xi8, strided<[1], offset: 771>>
    ...
    ^bb0(%reach_index: index):
      %val = memref.load %parent_origin[%reach_index] : memref<8xi8, strided<[1], offset: 771>>
      func.call @use(%val) : (i8) -> ()
      %next_reach_index = arith.subi %reach_index, %c1 : index
      scf.yield %next_reach_index : index
    }
    scf.for %i = %c0 to %c8 step %c1 {
      %val = memref.load %parent_origin[%i] : memref<8xi8, strided<[1], offset: 771>>
      func.call @use(%val) : (i8) -> ()
    }
    func.call @exit(%test_success) : (i32) -> ()

    memref.dealloc %parent : memref<779xi8>
    return %c0_i32 : i32
  }
