...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 859 : index
    %c0_i32 = arith.constant 0 : i32
    // locals

    %parent = memref.alloca() : memref<867xi8>

    %parent_origin = memref.subview %parent[0][8][1] : memref<867xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_target = memref.subview %parent[859][8][1] : memref<867xi8> to memref<8xi8, strided<[1], offset: 859>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_target[%i] : memref<8xi8, strided<[1], offset: 859>>
    }
    %distance_negated = arith.subi %c0, %distance : index
    %use_val_parent_target = memref.load %parent_target[%c0] : memref<8xi8, strided<[1], offset: 859>>
    ...
    ^bb0(%reach_index: index):
      %val = memref.load %parent_target[%reach_index] : memref<8xi8, strided<[1], offset: 859>>
      func.call @use(%val) : (i8) -> ()
      %next_reach_index = arith.addi %reach_index, %c1 : index
      scf.yield %next_reach_index : index
    }
    scf.for %i = %c0 to %c8 step %c1 {
      %val = memref.load %parent_target[%i] : memref<8xi8, strided<[1], offset: 859>>
      func.call @use(%val) : (i8) -> ()
    }
