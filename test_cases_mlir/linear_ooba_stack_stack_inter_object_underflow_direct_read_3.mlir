...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 944 : index
    %c0_i32 = arith.constant 0 : i32
    // locals

    %parent = memref.alloca() : memref<952xi8>

    %parent_target = memref.subview %parent[0][8][1] : memref<952xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_origin = memref.subview %parent[944][8][1] : memref<952xi8> to memref<8xi8, strided<[1], offset: 944>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_target[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 944>>
    }
    ...
    %use_val_parent_origin = memref.load %parent_origin[%c0] : memref<8xi8, strided<[1], offset: 944>>
    func.call @use(%use_val_parent_origin) : (i8) -> ()
    scf.for %reach_index = %c0 to %distance step %c1 {
      %index = arith.subi %c0, %reach_index : index
      %val = memref.load %parent_origin[%index] : memref<8xi8, strided<[1], offset: 944>>
      func.call @use(%val) : (i8) -> ()
    }
    scf.for %i = %c0 to %c8 step %c1 {
      %val = memref.load %parent_origin[%i] : memref<8xi8, strided<[1], offset: 944>>
      func.call @use(%val) : (i8) -> ()
    }
