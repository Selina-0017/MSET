...
module {
  ...
  memref.global @parent : memref<282xi8> = dense<170>

  func.func @f() -> i32 {
    %precond_fail = arith.constant 43 : i32
    %test_success = arith.constant 42 : i32
    %c0 = arith.constant 0 : index
    %distance = arith.constant 274 : index
    ...
    %parent = memref.get_global @parent : memref<282xi8>
    %parent_target = memref.subview %parent[0][8][1] : memref<282xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_origin = memref.subview %parent[274][8][1] : memref<282xi8> to memref<8xi8, strided<[1], offset: 274>>
    ...
    %use_val_parent_origin = memref.load %parent_origin[%c0] : memref<8xi8, strided<[1], offset: 274>>
    ...
    ^bb0(%reach_index: index):
      %val = memref.load %parent_origin[%reach_index] : memref<8xi8, strided<[1], offset: 274>>
      func.call @use(%val) : (i8) -> ()
      %next_reach_index = arith.subi %reach_index, %c1 : index
      scf.yield %next_reach_index : index
    }
    scf.for %i = %c0 to %c8 step %c1 {
      %val = memref.load %parent_origin[%i] : memref<8xi8, strided<[1], offset: 274>>
      func.call @use(%val) : (i8) -> ()
    }
