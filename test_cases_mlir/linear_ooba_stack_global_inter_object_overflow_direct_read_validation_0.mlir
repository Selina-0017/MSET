...
module {
  ...
  func.func @f() -> i32 {
    ...
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_target[%i] : memref<8xi8, strided<[1], offset: 858>>
    }
    ...
    scf.for %reach_index = %c0 to %c0 step %c1 {
      %val = memref.load %parent_target[%reach_index] : memref<8xi8, strided<[1], offset: 858>>
      func.call @use(%val) : (i8) -> ()
    }
    scf.for %i = %c0 to %c8 step %c1 {
      %val = memref.load %parent_target[%i] : memref<8xi8, strided<[1], offset: 858>>
      func.call @use(%val) : (i8) -> ()
    }
