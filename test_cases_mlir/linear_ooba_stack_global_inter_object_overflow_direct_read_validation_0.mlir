...
module {
  ...
  func.func @f() -> i32 {
    ...
    %use_val_parent_target = memref.load %parent_target[%c0] : memref<8xi8, strided<[1], offset: 95>>
    ...
    scf.for %reach_index = %c0 to %c0 step %c1 {
      %val = memref.load %parent_target[%reach_index] : memref<8xi8, strided<[1], offset: 95>>
      func.call @use(%val) : (i8) -> ()
    }
    scf.for %i = %c0 to %c8 step %c1 {
      %val = memref.load %parent_target[%i] : memref<8xi8, strided<[1], offset: 95>>
      func.call @use(%val) : (i8) -> ()
    }
