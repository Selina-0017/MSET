...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 1000 : index
    %c0xFF = arith.constant 255 : i8
    %c0_i32 = arith.constant 0 : i32
    // locals


    %parent = memref.alloc() : memref<1008xi8>
    %parent_origin = memref.subview %parent[0][8][1] : memref<1008xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_target = memref.subview %parent[1000][8][1] : memref<1008xi8> to memref<8xi8, strided<[1], offset: 1000>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_target[%i] : memref<8xi8, strided<[1], offset: 1000>>
    }
    %distance_negated = arith.subi %c0, %distance : index
    %use_val_parent_target = memref.load %parent_target[%c0] : memref<8xi8, strided<[1], offset: 1000>>
    ...
    memref.dealloc %parent : memref<1008xi8>
    return %c0_i32 : i32
  }
