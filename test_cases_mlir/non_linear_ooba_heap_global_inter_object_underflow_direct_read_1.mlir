...
module {
  ...
  func.func @f() -> i32 {
    ...
    %parent_target = memref.subview %parent[0][8][1] : memref<690xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_origin = memref.subview %parent[682][8][1] : memref<690xi8> to memref<8xi8, strided<[1], offset: 682>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_target[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 682>>
    }
    %distance_negated = arith.subi %c0, %distance : index
    scf.for %j = %c0 to %c8 step %c1 {
      %idx = arith.addi %j, %distance : index
      %val = memref.load %parent_origin[%idx] : memref<8xi8, strided<[1], offset: 682>>
