...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 160 : index
    %c0xFF = arith.constant 255 : i8
    %c0_i32 = arith.constant 0 : i32
    // locals

    %parent = memref.alloca() : memref<168xi8>

    %parent_origin = memref.subview %parent[0][8][1] : memref<168xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_target = memref.subview %parent[160][8][1] : memref<168xi8> to memref<8xi8, strided<[1], offset: 160>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_target[%i] : memref<8xi8, strided<[1], offset: 160>>
    }
    %distance_negated = arith.subi %c0, %distance : index
    %use_val_parent_target = memref.load %parent_target[%c0] : memref<8xi8, strided<[1], offset: 160>>
    ...
    ^bb0(%reach_index: index):
      memref.store %c0xFF, %parent_target[%reach_index] : memref<8xi8, strided<[1], offset: 160>>
      %next_reach_index = arith.addi %reach_index, %c1 : index
      scf.yield %next_reach_index : index
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xFF, %parent_target[%i] : memref<8xi8, strided<[1], offset: 160>>
    }
