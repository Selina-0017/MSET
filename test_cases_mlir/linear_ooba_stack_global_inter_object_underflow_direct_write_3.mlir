...
module {
  ...
  func.func @f() -> i32 {
    ...
    %c0xFF = arith.constant 255 : i8
      %__base = arith.constant 695 : index
    %c0_i32 = arith.constant 0 : i32
    // locals

    %parent = memref.alloca() : memref<703xi8>

    %parent_target = memref.subview %parent[0][8][1] : memref<703xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_origin = memref.subview %parent[695][8][1] : memref<703xi8> to memref<8xi8, strided<[1], offset: 695>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_target[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 695>>
    }
    %distance_negated = arith.subi %c0, %distance : index
    scf.for %reach_index = %c0 to %distance step %c1 {
      %index = arith.subi %c0, %reach_index : index
      memref.store %c0xFF, %parent_origin[%index] : memref<8xi8, strided<[1], offset: 695>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      %__idx = arith.addi %i, %__base : index
      memref.store %c0xFF, %parent_origin[%__idx] : memref<8xi8, strided<[1], offset: 695>>
    }
