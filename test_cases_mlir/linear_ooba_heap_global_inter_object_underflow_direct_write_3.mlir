...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 808 : index
    %c0xFF = arith.constant 255 : i8
    %c0_i32 = arith.constant 0 : i32
    // locals


    %parent = memref.alloc() : memref<816xi8>
    %parent_target = memref.subview %parent[0][8][1] : memref<816xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_origin = memref.subview %parent[808][8][1] : memref<816xi8> to memref<8xi8, strided<[1], offset: 808>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_target[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 808>>
    }
    ...
    %use_val_parent_origin = memref.load %parent_origin[%c0] : memref<8xi8, strided<[1], offset: 808>>
    ...
    scf.for %reach_index = %c0 to %negadist_variant step %c1 {
      ...
      memref.store %c0xFF, %parent_origin[%index] : memref<8xi8, strided<[1], offset: 808>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xFF, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 808>>
    }
    func.call @exit(%test_success) : (i32) -> ()

    memref.dealloc %parent : memref<816xi8>
    return %c0_i32 : i32
  }
