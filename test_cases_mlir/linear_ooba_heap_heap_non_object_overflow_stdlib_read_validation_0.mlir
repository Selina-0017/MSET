...
module {
  ...
  func.func @f() -> i32 {
    ...
    %read_value_118 = memref.alloca() : memref<1024xi8>
    scf.for %i = %c0 to %c0 step %c1024 {
      ...
      %dst_slice = memref.subview %read_value_118[%c0][%step][1] : memref<1024xi8> to memref<?xi8, strided<[1], offset: ?>>
      memref.copy %src_slice, %dst_slice : memref<?xi8, strided<[1], offset: ?>> to memref<?xi8, strided<[1], offset: ?>>
    }
    %use_val_read_value_118 = memref.load %read_value_118[%c0] : memref<1024xi8>
    func.call @use(%use_val_read_value_118) : (i8) -> ()
    %read_value_119 = memref.alloca() : memref<8xi8>
    memref.copy %origin, %read_value_119 : memref<8xi8> to memref<8xi8>
    %use_val_read_value_119 = memref.load %read_value_119[%c0] : memref<8xi8>
    func.call @use(%use_val_read_value_119) : (i8) -> ()
