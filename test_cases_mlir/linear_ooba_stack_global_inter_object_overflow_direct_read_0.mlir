...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 631 : index
    %c0_i32 = arith.constant 0 : i32
    // locals

    %parent = memref.alloca() : memref<639xi8>

    %parent_origin = memref.subview %parent[0][8][1] : memref<639xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_target = memref.subview %parent[631][8][1] : memref<639xi8> to memref<8xi8, strided<[1], offset: 631>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_target[%i] : memref<8xi8, strided<[1], offset: 631>>
    }
    %distance_negated = arith.subi %c0, %distance : index
    %use_val_parent_target = memref.load %parent_target[%c0] : memref<8xi8, strided<[1], offset: 631>>
