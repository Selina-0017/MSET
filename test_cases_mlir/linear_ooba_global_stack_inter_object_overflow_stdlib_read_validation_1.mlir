...
module {
  ...
  func.func @f() -> memref<8xi8> {
    ...
    %read_value_244 = memref.alloca() : memref<1024xi8>
    ...
    scf.for %i = %c0 to %c0 step %c1024 {
      ...
      %dst_slice = memref.subview %read_value_244[%c0][%step][1] : memref<1024xi8> to memref<?xi8, strided<[1], offset: ?>>
      memref.copy %src_slice, %dst_slice : memref<?xi8, strided<[1], offset: ?>> to memref<?xi8, strided<[1], offset: ?>>
    }
    %use_val_read_value_244 = memref.load %read_value_244[%c0] : memref<1024xi8>
    func.call @use(%use_val_read_value_244) : (i8) -> ()
    %read_value_245 = memref.alloca() : memref<8xi8>
    memref.copy %target, %read_value_245 : memref<8xi8> to memref<8xi8>
