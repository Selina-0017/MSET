...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 884 : index
    %c0xFF = arith.constant 255 : i8
    %c0_i32 = arith.constant 0 : i32
    // locals


    %parent = memref.alloc() : memref<892xi8>
    %parent_target = memref.subview %parent[0][8][1] : memref<892xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_origin = memref.subview %parent[884][8][1] : memref<892xi8> to memref<8xi8, strided<[1], offset: 884>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_target[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 884>>
    }
    ...
    %use_val_parent_origin = memref.load %parent_origin[%c0] : memref<8xi8, strided<[1], offset: 884>>
    ...
    ^bb0(%reach_index: index):
      memref.store %c0xFF, %parent_origin[%reach_index] : memref<8xi8, strided<[1], offset: 884>>
      %next_reach_index = arith.subi %reach_index, %c1 : index
      scf.yield %next_reach_index : index
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xFF, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 884>>
    }
    func.call @exit(%test_success) : (i32) -> ()

    memref.dealloc %parent : memref<892xi8>
    return %c0_i32 : i32
  }
