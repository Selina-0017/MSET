...
module {
  ...
  func.func @f() -> i32 {
    ...
    %distance = arith.constant 548 : index
    %c0_i32 = arith.constant 0 : i32
    // locals


    %parent = memref.alloc() : memref<556xi8>
    %parent_target = memref.subview %parent[0][8][1] : memref<556xi8> to memref<8xi8, strided<[1], offset: 0>>
    %parent_origin = memref.subview %parent[548][8][1] : memref<556xi8> to memref<8xi8, strided<[1], offset: 548>>
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xAA, %parent_target[%i] : memref<8xi8, strided<[1], offset: 0>>
    }
    scf.for %i = %c0 to %c8 step %c1 {
      memref.store %c0xBB, %parent_origin[%i] : memref<8xi8, strided<[1], offset: 548>>
    }
    ...
    %use_val_parent_origin = memref.load %parent_origin[%c0] : memref<8xi8, strided<[1], offset: 548>>
    ...
    scf.for %reach_index = %c0 to %negadist_variant step %c1 {
      ...
      %val = memref.load %parent_origin[%index] : memref<8xi8, strided<[1], offset: 548>>
      func.call @use(%val) : (i8) -> ()
    }
    scf.for %i = %c0 to %c8 step %c1 {
      %val = memref.load %parent_origin[%i] : memref<8xi8, strided<[1], offset: 548>>
      func.call @use(%val) : (i8) -> ()
    }
    func.call @exit(%test_success) : (i32) -> ()

    memref.dealloc %parent : memref<556xi8>
    return %c0_i32 : i32
  }
